...
def apply_transform(transform, data):
    ...
    return  [ (d[p[0]]*sx,d[p[1]]*sy,d[p[2]]*sz) for d in data ]

def manhattan_diam(data):
    return max(
...
class ScanData:
    ...
    def __init__(self, data, scanners=[(0,0,0)]):
        self.data = data
    ...
    def data_transforms(self):
        ...
        for transform in product(axis_perms,sgns,sgns,sgns):
            p, sx,sy,sz = transform
            yield transform, apply_transform(transform, self.data)
    ...
    def try_match(self, oth, min_overlap=12):
        ...
        for (i,j) in pt_pairs:
            for transform, oth_view in oth.data_transforms():
                y0 = self.data[i]
                x0 = oth_view[j]
                oth_x0_set = set(
                ...
                if n_matches >= min_overlap:
                    merged_data = list(oth_x0_set | self_set) 
                    merged_scanners = [
                        tuple(y0i + (si-x0i) for si,x0i,y0i in zip(s,x0,y0))
                        for s in apply_transform(transform,oth.scanners)
                    ] + self.scanners
                    return ScanData(merged_data, merged_scanners)
...
def main():
    ...
    print('Manhattan Diameter:', manhattan_diam(sd[0].scanners))
